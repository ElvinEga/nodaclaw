...
use agent_api::adapters::openclaw::compact_recall_context;
use agent_api::{
    AgentApiService, OutcomeRecordDto, ProposeMemoryRequest, RecallContextRequest,
    RecordOutcomeRequest,
};
use chrono::Utc;
use memory_core::{Edge, MemoryPacket, MemoryStatus, Node, NodeId};
use memory_ingest::{AdmissionContext, IngestEvent, MessageEvent};
...
impl NodamemAdapter {
    ...
    pub async fn recall_context(&self, request: &RecallRequest) -> anyhow::Result<Option<RecallResult>> {
    ...
    pub async fn propose_memory(
        ...
        for node in response.ingest_output.candidate_nodes.iter().filter(|candidate| {
            accepted_ids.contains(&candidate.id)
        }) {
        ...
        let persisted_node_ids: HashSet<NodeId> = runtime
            ...
            .map(|node| node.id)
            .collect();
        for edge in response.ingest_output.candidate_edges.iter().filter(|edge| {
...
fn format_compact_context(response: &agent_api::adapters::openclaw_types::OpenClawRecallContextResponse) -> String {
    let mut lines = vec![
